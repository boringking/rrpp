...
int rrpp_regs_config(void){
	...
	if(singleRegSetting(ID_COMMON_MISC_SET_REG,0x0C,0x04,SPECIAL_TAG_VAL)<0)
		goto WRITE_FAILED;

	/* enable special tag function and CPU port(P29)*/
	if(singleRegGetting(ID_COMMON_MISC_GET_REG,0x0C,0x03,&rd_reg)<0)
		goto READ_FAILED;
	if(singleRegSetting(ID_COMMON_MISC_SET_REG,0x0C,0x03,rd_reg|(3ul<<0))<0)
		goto WRITE_FAILED;

	/* disable BPDU per Port setting */
	if(singleRegGetting(ID_COMMON_MISC_GET_REG,0x00,0x33,&rd_reg)<0)
		goto READ_FAILED;
	if(singleRegSetting(ID_COMMON_MISC_SET_REG,0x00,0x33,rd_reg&(~(1ul<<8)))<0)
		goto WRITE_FAILED;
