...
static void __fill_rrpp_frame(struct rrpp_packet_without_vlan * frame){
	...
	// pre_rrpp_field
	frame->pre_rrpp.frame_length = htons(sizeof(struct pre_rrpp_field)+sizeof(struct rrpp_field)-2);
...
static void __fill_vlan_rrpp_frame(struct rrpp_packet_with_vlan * frame , u16 vid){
...
//	__fill_vlan_rrpp_frame(&rrpp_pkt , atoi(argv[1]));
    ...
    for(int i=0 ; ;++i){  
        int ret=raw_socket_sendto(fd,&pkt,sizeof(pkt),&ll_addr);   //将构造的UDP帧发送到数据链路层  
        if(ret<0){  
            perror("error");  
			return -1;
        }  
