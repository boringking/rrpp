 ...
 */
typedef struct dhcpv6_packet {
	unsigned char msg_type;
	unsigned char transaction_id[3];
	unsigned char options[1];
}m_dhcpv6_packet;

typedef struct _DUID_LLT{
	u_int16_t duidtype;
	u_int16_t hardwaretype;
	u_int32_t duidtime;
	u_int8_t	clientmac[6];
}m_DUID_LLT;

typedef struct _DUID_LL{
	u_int16_t duidtype;
	u_int16_t hardwaretype;
	u_int8_t	clientmac[6];
}m_DUID_LL;

typedef struct _IA_ADDR{
	u_int16_t type;
	u_int16_t len;
	u_int8_t addr[16];
	u_int32_t preferTime;
	u_int32_t validTime;
}m_IA_ADDR;

typedef struct _IA_TA{
	u_int32_t iaid;
	m_IA_ADDR ia_addr;
}m_IA_TA;
...
#define DHCPV6_RELAY_HDR_LEN 34

/*
 * Relay packet format, defined in section 7 of RFC 3315
 */
typedef struct dhcpv6_relay_packet {
	unsigned char msg_type;
	unsigned char hop_count;
	unsigned char link_address[16];
	unsigned char peer_address[16];
	unsigned char options[1];
}m_dhcpv6_relay_packet;
...
typedef struct dhcpv6_relaymsg {
	u_int16_t msg_type;
	u_int16_t msg_len;
	u_int8_t	*msg_value;
}m_dhcpv6_relaymsg;

#define DHCPV6_OPTION_IFACE_ID_LEN 8
typedef struct dhcpv6_op_ifaceid {
	u_int16_t msg_type;
	u_int16_t msg_len;
	u_int32_t iface_id;
}m_dhcpv6_op_ifaceid;
...
extern uint8_t switch_ipv6[IPV6_ADDRESS_LEN];
#endif
