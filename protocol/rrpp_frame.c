...
int is_rrpp_dst_mac (const uint8_t * mac) {
    ...
    if ( !memcmp (mac, rrpp_dst_mac, 5) && mac[5] >= RRPP_DST_MAC_B5_BEGIN
         && mac[5] <= RRPP_DST_MAC_B5_END) {
        return true;
    }
...
static void fill_rrpp_frame (
    ...
    frame->pre_rrpp.frame_length = htons (sizeof (struct pre_rrpp_field) + sizeof (
            struct rrpp_field) - sizeof (frame->pre_rrpp.frame_length) );
    ...
    frame->rrpp.rrpp_length = sizeof (struct rrpp_field) - sizeof (
