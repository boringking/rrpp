...
int transport_node_constructor (
    ...
    if (thread_constructor (&this->detect_link_status_thread, "detect-link-thread",
                            DEFAULT_STACK_SIZE, transport_node_detect_link_status_thread, this) < 0) {
        goto THREAD_CONSTRUCTOR_FAIL;
    }

    if (thread_constructor (&this->recv_thread, "recv-thread", DEFAULT_STACK_SIZE,
                            transport_node_recv_thread, this) < 0) {
        goto THREAD_CONSTRUCTOR_FAIL;
    }

    if (thread_constructor (&this->recv_complete_thread, "recv-complete-thread",
                            DEFAULT_STACK_SIZE, transport_node_recv_complete_thread, this) < 0) {
        goto THREAD_CONSTRUCTOR_FAIL;
    }

    if (thread_constructor (&this->check_complete_thread, "check-complete-thread",
                            DEFAULT_STACK_SIZE, transport_node_check_complete_thread, this) < 0) {
        goto THREAD_CONSTRUCTOR_FAIL;
    }
...
// 检查rrpp报文类型,并获得端口号
int transport_node_judge_frame_type (struct transport_node * this,
                                     const void * frame, int * port) {
    const struct rrpp_special_vlan_packet * pkt = (const struct
            rrpp_special_vlan_packet *) frame;
    ...
    */
    if ( TRANSPORT_DOWN == this->status && true == handle_port_link) {
        if ( true == handle_port_link && true == couple_port_link) {
            ...
            fill_rrpp_linkup_frame (&frame, this->raw_sock.local_mac, this->vlan->vid,
                                    this->did, this->rid);
        ...
        fill_rrpp_linkdown_frame (&frame, this->raw_sock.local_mac, this->vlan->vid,
                                  this->did, this->rid);
        this->sendto_port (this, couple_portno, &frame, sizeof frame);
    }
}
...
void * transport_node_recv_thread (void * arg) {
    ...
    for (int cnter = 1;; ++cnter) {
        ...
        /* 接收报文 */
        if ( (recv_len = raw_socket_recvfrom (&this->raw_sock, buf, sizeof (buf), &addr,
                                              &addrlen) ) < 0) {
            perror ("recvfrom");
            return NULL;
        }
...
int transport_node_sendto_port (struct transport_node * this, int port,
                                const void * src, int len) {
    return sendto_port (&this->raw_sock, port, src, len);
}

int transport_node_block_port (struct transport_node * this , int port ,
                               const vlan_t * exclude) {
    return block_port (this, port, exclude);
}

int transport_node_release_port (struct transport_node * this , int port ,
                                 const vlan_t * exclude) {
