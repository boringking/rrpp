...
int major_node_constructor (
    ...
    if (thread_constructor (&this->recv_thread, "recv-thread", DEFAULT_STACK_SIZE,
                            major_node_recv_thread, this) < 0) {
        goto THREAD_CONSTRUCTOR_FAIL;
    }

    /* 定时发送hello报文线程 */
    if (thread_constructor (&this->hello_thread, "hello-thread", DEFAULT_STACK_SIZE,
                            major_node_hello_thread, this) < 0) {
        goto THREAD_CONSTRUCTOR_FAIL;
    }

    /* 检查接收hello报文超时线程 */
    if (thread_constructor (&this->check_hello_thread, "check-hello-thread",
                            DEFAULT_STACK_SIZE, major_node_check_hello_thread, this) < 0) {
        goto THREAD_CONSTRUCTOR_FAIL;
    }
...
void * major_node_check_hello_thread (void * arg) {
    struct major_node * this = (struct major_node *) arg;
    struct rrpp_vlan_packet frame;
    fill_rrpp_complete_flush_fdb_frame (&frame, this->raw_sock.local_mac,
                                        this->vlan->vid, this->did, this->rid);
...
// 检查rrpp报文类型,并获得端口号
int major_node_judge_frame_type (struct major_node * this, const void * frame,
                                 int * port) {
...
void major_node_recv_link_down (struct major_node * this, int port) {
    ...
    if (action) {
        ...
        struct rrpp_vlan_packet frame;
        fill_rrpp_common_flush_fdb_frame (&frame, this->raw_sock.local_mac,
                                          this->vlan->vid, this->did, this->rid);
        this->sendto_port (this, this->main_portno, &frame, sizeof frame);
        this->sendto_port (this, this->second_portno, &frame, sizeof frame);
...
void major_node_recv_link_up (struct major_node * this, int port) {
    ...
    if (action) {
        ...
        fill_rrpp_hello_frame (&hello, this->raw_sock.local_mac, this->vlan->vid,
                               this->did, this->rid);
...
/* 像指定端口发送报文 */
int major_node_sendto_port (struct major_node * this, int port,
                            const void * src, int len) {
    return sendto_port (&this->raw_sock, port, src, len);
}

/* 阻塞指定端口 */
int major_node_block_port (struct major_node * this , int port ,
...
/* 释放指定端口 */
int major_node_release_port (struct major_node * this , int port ,
                             const vlan_t * exclude) {
